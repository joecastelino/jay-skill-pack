...
- **himalaya IMAP auth can die independently of Tekion (verified 2026-08-07) — login.py hangs silently instead of failing fast.** Symptom: `login.py --force` prints `[login] OTP baseline count = 0` then `no fresh OTP yet; attempting Resend fallback` then `FAIL: no fresh OTP within timeout` — repeatedly, on every retry, even though email+password submission worked fine. Root cause was NOT Tekion or OTP timing — it was `himalaya envelope list -a personal -f "[Gmail]/All Mail"` itself failing with `AUTHENTICATIONFAILED — Invalid credentials (Failure)` (the Gmail App Password in `~/.config/himalaya/config.toml` had been revoked/rotated). `count_otp_emails()` swallows this exception and just returns 0, so login.py can never see a fresh OTP arrive and loops through Resend/timeout forever with a MISLEADING "no fresh OTP" message that looks like a Tekion-side delay. **Fast diagnostic — run this FIRST before retrying login.py a second time:** `HOME=/home/itadmin himalaya envelope list -a personal -f "[Gmail]/All Mail" -s 15` — if this errors with AUTHENTICATIONFAILED, the problem is Gmail credentials, not Tekion/OTP timing, and NO amount of login.py retrying will fix it. This is a HARD BLOCKER Jay cannot self-heal — a new Gmail App Password must be generated by Joe (Google Account → Security → App Passwords) and dropped into the himalaya config; flag it immediately rather than burning retries. Also check the OTHER Gmail path (`~/.hermes/google_token.json`, used by `google_api.py` for Sheets/Gmail API) separately — its OAuth refresh_token can be revoked independently (`invalid_grant: Token has been expired or revoked`) even while himalaya still has its own separate app-password credential; the two auth paths fail independently, always check both before concluding "Gmail is down."

  **RESOLUTION (verified 2026-08-07):** when Joe generates a new App Password, he
  may hand it to a DIFFERENT agent (e.g. Stacey/email-agent) rather than direct
  to Jay — Jay must still independently patch his OWN himalaya config, since each
  profile/agent has its own separate `~/.config/himalaya/config.toml` (this box:
  `/home/itadmin/.config/himalaya/config.toml`, account `personal`). Steps:
  1. Patch BOTH `raw = "..."` lines (under `[accounts.personal.backend.auth]` and
     `[accounts.personal.message.send.backend.auth]`) with the new 16-char app
     password (strip spaces if Joe pastes it space-separated, e.g.
     "rsjp fdyi gthg elwr" → "rsjpfdyigthgelwr").
  2. Verify immediately: `HOME=/home/itadmin himalaya envelope list -a personal
     -f "[Gmail]/All Mail" -s 5` — should list envelopes with no AUTHENTICATIONFAILED.
  3. Re-run `login.py --force` — it will now find the fresh OTP baseline > 0 and
     succeed within ~20s (no more "OTP baseline count = 0" / Resend-timeout loop).
  Do NOT wait passively for Joe to say "try again" a second time before verifying
  himalaya directly — test himalaya the moment Joe indicates ANY credential change
  happened (even if he says he gave it to another agent), since the fix must be
  applied per-config-file, not fleet-wide.
