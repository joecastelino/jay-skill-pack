...
| **Pay Types Setup** (Service 3.0 — split payers from pay types) | App Grid → Settings → Service Settings → Pay Types Setup, or direct `/ro/paytypes`. "Add New" (top-right) → Base Pay Type (Customer/Internal/Warranty) + 3-letter Notation + Name + Default Payer + Associated Payers (multi-select). Base C/I/W pay types can't be deactivated, only custom ones. Requires "View Pay Types Setup" permission (Roles → Permissions → Service → RO) — **without it the App Grid tile is HIDDEN (search = "No Match Found") and direct nav renders "You do not have the permissions to access the content"** (hit live at VC 1891, System Administrator role, 2026-09-02 — perm was OFF even for sysadmin). If Joe says "it's not there", check the permission FIRST. Step 1 of the Service 3.0 manager setup chain: Pay Types → Tax Codes → Vehicle Groups → Labor Pricing → Opcode pay-type config → Customer Mgmt defaults → Fees → GLAM cash holding account. Verified 2026-08-14. |
...
| **PDF Settings** (invoice print copies, auto-print triggers, PDF Configurator) | ✅ **`/ro/pdf-settings`** (verified VC 2026-09-02). Left-nav: Default / PDF Configurator / **Print** / Content. Print tab = "Print on invoice" toggle + AutoPrint Settings react-table (cols: Document·Category·Auto Print Trigger(s)·Check-in Auto Print·Duplex·Display Voided Job·Reprint·Job Tag·Job External Note·**Quantity**·Quick RO Auto Print·VI RO Auto Print). **Quantity = copies per print event → the "invoice prints twice" root cause.** ⚠ `/service/settings/pdf-settings` and `/pdf-settings` render blank/bounce. ⚠ `/click text="Print"` matches a Reports link and bounces to `/core/reports` — tag the left-nav leaf (exact text 'Print', x<400, y>50) with data-jay first. Quantity column is off-screen right; read rows via DOM (`.rt-td` innerText), not vision. See skill `tekion-pdf-print-copies-diagnosis`. |
...
| **Cashiering (Service/Parts invoices)** | **`/ro/invoices`** (left-rail "C" icon; redirects to `/ro/invoices/REPAIR_ORDER`). ⚠ `/service/cashiering` and `/cashiering` render a blank shell or bounce to /home — do NOT use. Tabs Service\|Parts; filter Status Pending/Paid. Open an invoice: click the RO# cell (`.roid-wrapper.pointer`) → detail URL `/ro/invoices/REPAIR_ORDER/<objectId>`. Detail page innerText is only ~830 chars when FULLY rendered (don't apply the "<700 chars = not loaded" RO-page heuristic here) — payment section shows Due Amount + Mode of Payment (Credit/Debit Card, Cash, Check, OEM Reward, Other, Gift Card). "OEM Reward" = generic MANUAL tender (blank Amount/ID/Date fields) unless the My GM Rewards 2.0 integration is enabled (see skill `tekion-oem-rewards-integration`). Verified BC 2026-09-03. |
...
| **Accounting Global Settings** | ✅ **`/accounting/accountSettings`** (camelCase, VERIFIED dealer 1891 2026-09-18). Tabs: Journal Entry Settings · **Auto Posting** · Schedules · Reconciliation Accounts · General Settings. Auto Posting sub-tabs: **Posting Preferences** (≈180,158) / **Posting Templates** (≈405,158); template sub-tabs Repair Order (≈221,748) · Parts Sale (≈221,820) · Warranty Credit (≈221,892). Holds "Posting Preferences → Repair Orders Close (CP/Internal/Warranty) = Auto-Post vs Draft", the **"GL Accounts Not Requiring Sales Chain"** declaration list, and the "Send RO/SO postings to error if vehicle inventory account balance is zero" toggle. |
| Sales Chain | ✅ `/accounting/accountingChain/list` (e.g. `4403`/`4703` = No rows found at VC; every normal sale acct has a chain row.) |
| **CoA account edit** | ✅ `/accounting/chartOfAccounts/dealer/<dealerId>/account/<dealerId>_<acct>/edit` — e.g. `.../dealer/1891/account/1891_4403/edit`. Tabs: Account Details · Postings · Monthly Balances · Daily Balances. Flag toggles are `.ant-switch`: Control Number Mandatory · Control 2 Mandatory · **Description Mandatory** · Enable Posting Line Count Adjustment · **Disable Postings**. Note **`Modified Time` is a nightly balance-recompute stamp, NOT an edit date** — every account reads the same morning time. |
| Coupons | List `/core/coupons` (read the `N Result(s)` block); edit page `/core/coupons/edit/<base64url(code)>` (also carries the coupon's **GL Account Split** = the discount accounts). Search box = the expandable magnifier (≈1155,203). |
| **Release Notes (what shipped)** | DMS **Home → Release Notes card**; the **`Read All (N)` is an `<a href="/core/release/<24-hex-id>">`** — read the href (clicking / `window.open` hooking both fail, the drawer never expands). Department filter is a URL param: `?departmentId=ACCOUNTING\|CORE\|SERVICE\|SALES\|PARTS\|TEKION_PAY\|ANALYTICS\|COMMUNICATIONS\|PERMISSIONS`; body text carries `Released On: <date>`. **The KB has NO ARC release notes** — this widget is the only source. |
...
`/accounting/global-settings`, `/accounting/globalSettings/list`, `/accounting/accounting-global-settings`, `/accounting/settings/global`, `/accounting/settings`, `/accounting/globalSetting/list` → **all silently redirect to `chartOfAccounts/list`** (the real route is `/accounting/accountSettings` — found by harvesting anchors off a KB article, see below).

**Route-discovery trick (use before guessing URLs):** open the KB article that documents the screen, then in the SPA tab run
`[...document.querySelectorAll('a')].map(a=>a.getAttribute('href')).filter(h=>/tekioncloud/.test(h||''))`
— KB bodies link the live screen paths. This is how `/accounting/accountSettings` and `/accounting/accountingChain/list` were recovered after URL guessing failed.

**Table-level expandable search (CoA / Sales Chain / JE list / coupons):** the **global** search box has placeholder `Search here...` (~700,75) and does **not** filter these tables — typing there returns the unfiltered list and reads as a false negative. The real filter is a **magnifier icon at the RIGHT edge of the table header** (~x1159 CoA, x1224 VC Sales Chain, x1111 JE list) which expands an input with placeholder exactly **`Search...`**. Type with the native value-setter + `input` event + Enter.
