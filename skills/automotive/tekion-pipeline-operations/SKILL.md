...
**⚠️ SELF-INFLICTED CHRONIC DRAIN — the #1 suspect before blaming Tekion (found\n2026-08-05, SCT outage that had run 40+ hours):** Before assuming a DEALER_QUOTA\noutage is Tekion-side, check whether JAY'S OWN recovery watchers are the actual\ncause. Pattern found: dated one-off watcher scripts named\n`selfheal_sct_align_YYYYMMDD.sh` (and `wait_ops_then_scrape_*.sh` variants) had\nbeen auto-relaunched under a NEW date almost every day going back to **July 7** —\neach one polling the exhausted endpoint every 10-15 min, forever, with no\n\"don't duplicate/don't relaunch if one already ran today\" guard. Each poll cycle\nfires a 6-call burst (paged RO search x4 + Get Jobs + Get Operations) that lands\ndirectly on the same already-exhausted per-dealer bucket. If 429 responses still\ncount against the quota counter (typical for API gateways), a never-ending retry\nloop can prevent the bucket from ever getting a clean window to refill — meaning\nJay's own self-heal infrastructure IS the reason the outage never clears, and\nonly SCT gets hit this hard because SCT is the one store with automated\nalignment/menu-sales recovery watchers; the other 6 stores have none, which is\nalso why an outage is so often SCT-only.\n\n**Diagnose:**\n```bash\npgrep -af \"selfheal_sct\\|wait_ops_then_scrape\\|self.*heal.*sct\"\n# also check for a supervisor/relaunch mechanism — these scripts came back\n# under a new date even after being kill -9'd earlier the same day, implying\n# something (gateway restart handler?) respawns them. Not fully traced yet.\n```\n**Fix when found:** `kill -9` every matching PID, confirm none respawn within a\nfew minutes, then leave the store COMPLETELY untouched (no polling at all) for\n30-60 min before a single manual probe — if quota clears once truly left alone,\nthat confirms self-inflicted and rules out a Tekion-side problem.\n\n**Permanent fix needed (not yet built as of 2026-08-05):** replace the whole\nfamily of dated one-off `selfheal_sct_*_YYYYMMDD.sh` scripts with ONE permanent,\nnon-duplicating watcher that (a) checks for an already-running or already-ran-\ntoday instance before launching (lock file dated to the day, not the run), and\n(b) uses a MUCH longer poll interval (e.g. hourly, not every 10-15 min) so it\ncan't itself re-drain a bucket it's trying to let recover.\n\n**Ground-truth verification via the APC Transaction Dashboard (found\n2026-08-05) — use this instead of guessing from internal logs:** the APC portal\n(apc.tekioncloud.com, persistent browser :9224) has a real usage log at\n9-dot grid → **Transaction Dashboard** tile (URL ends in `/list`, NOT the\npaywalled \"Usage Dashboard\" tile which is inaccessible). It shows every API call\nwith Dealer Name, endpoint, HTTP status, timestamp. The UI's multi-select filter\ncheckboxes (Dealer Name, HTTP Status) are FLAKY under coordinate-based clicking —\ndon't fight them for long. Instead, hook the underlying API the dashboard itself\ncalls: `POST /api/apc-core/u/vendor/api-metrics/search` — drive it via in-page\n`fetch()` calls (creds/session attach automatically) with different filter\npayloads once you've captured one real request/response shape via an XHR/fetch\nhook. This is the fastest way to get a real answer to \"which store/endpoint is\nactually burning quota\" without touching internal scraper logs at all — and it's\nhow the self-inflicted-watcher root cause above was actually proven (the 6-call\nburst pattern repeating every 10-15 min was visible directly in this dashboard,\ntraced back to the dated selfheal scripts).
