...
- **STALE DRAFT (not just stale Sent copy) can carry an earlier run's WRONG total —
  a "DRAFT <id>" reply is not proof it's correct (hit 2026-08-13, 5 PM run).**
  When you ask "is there a draft for subject X?" and she replies "DRAFT 42212",
  that only confirms a draft with that subject EXISTS — it can still hold a
  STALE total baked in from an earlier same-day run (e.g. a noon run's
  $2,241.37 sitting in a draft created before the 5 PM re-scrape, while the
  correct 5 PM total was $2,066.93). Telling her to "just send draft 42212"
  would ship the WRONG numbers. → Always ask her to ALSO state the draft's
  dollar total in the same query ("DRAFT <id> — total in body?"), compare
  against today's opened-JSON total, and if it mismatches, have her DELETE
  that draft and build a fresh one from scratch (don't try to edit it) before
  sending. This is the same family as the "stale earlier-send collision" trap
  below but hits the DRAFT stage, not just the Sent-folder check — verify
  totals at BOTH stages (draft-exists check AND send-verify), not just one.
- **Jay's own terminal `himalaya` calls FAIL with IMAP auth error (hit
  2026-08-13)** — `himalaya envelope list -f "[Gmail]/Sent Mail"` run directly
  from Jay's profile returns `AUTHENTICATIONFAILED`. Jay's environment does not
...
9/4 noon lost too (DAY 4): search/jobs 200, /operations 429 DEALER_QUOTA,
7 TEK candidates out of 104 ROs unscannable (582961/582925/582918/582908/
582902/582891/582875). JSONs flagged complete:false, no render/email/watcher.
Also killed two leftover sct-align selfheal watchers (PIDs from 9/3) that were
still polling and burning the shared dealer quota; cleared their /tmp locks.
Day 4 continuous — Tekion support ticket escalation to Joe re-flagged.
9/4 5PM lost too (DAY 4 continues): search/jobs 200, /operations 429
DEALER_QUOTA, 9 TEK candidates out of 162 ROs unscannable (583016/582970 new +
the 7 from noon). JSONs flagged complete:false, no render/email/watcher. No
live watchers found; stale selfheal locks (9/2, 9/3) have no holders. Tekion
Tekion support ticket escalation to Joe stands — 4 straight days, 8 cron runs lost.
9/5 noon lost too (DAY 5): search/jobs 200, /operations 429 DEALER_QUOTA,
5 TEK candidates out of 97 ROs unscannable (583068/583061/583048/583043/583037).
JSONs flagged complete:false, no render/email/watcher. Killed two leftover 9/4
...
9/6 noon lost too (DAY 6): search/jobs 200, /operations 429 DEALER_QUOTA,
4 TEK candidates out of 74 ROs unscannable (583236/583224/583203/583196).
JSONs flagged complete:false, no render/email/watcher. Killed two leftover 9/5
selfheal watchers (PIDs 1525120/1525148 + sleep child) still burning the shared
