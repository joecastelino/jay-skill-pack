...
**Make split (Joe's canonical filter, confirmed 2026-08-03):** TOYOTA row = Make **is like `Toyota` AND `Scion`** (Scion counts as Toyota — 9 SCT July ROs would leak to Others without it); OTHERS row = Make **is not like** Toyota/Scion. In Tekion Reports UI (Advisor Performance): funnel icon → Add Filter → Make / is like / Toyota, Scion. In the DB: `payload->'vehicle'->>'make'` matched case-insensitively against `/toyota|scion/` (make casing is dirty: both "toyota" and "Toyota" appear). Script: `/home/itadmin/dealer-detail/apps/web/wip_makes.cjs`. For non-Toyota stores swap the make regex per store brand (VW stores: `volkswagen`; BC: `chevrolet|cadillac|gmc|buick`? — CONFIRM with Joe per store before first use).
