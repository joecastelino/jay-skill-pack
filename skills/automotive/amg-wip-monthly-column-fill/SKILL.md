...
- **PDI** = `PDI` — ⚠️ **UNRESOLVED as of 2026-08-03, do not use blindly.** The saved group literally named "PDI" is configured as Opcode **"Not In" PDI** + Pay Type **"In" Internal** — i.e. it EXCLUDES PDI opcodes and scopes to Internal pay type, the opposite of what the name implies. Applying it for July gave Bill Hrs 1,126.78 / RO Count 4,720 — implausibly high for a PDI-only bucket, more consistent with a general "Internal" catch-all. Flagged to Joe rather than guessing; do NOT report numbers from this group as PDI until Joe confirms whether to (a) flip Opcode to "In" PDI for a true PDI-only read, or (b) treat this group as the INTERNAL row instead and build a separate PDI filter. **Lesson: a saved group's NAME is not proof of its filter logic — always re-open the funnel popover and read the actual field/operator/value rows before trusting output, especially if the resulting count looks implausible.**
